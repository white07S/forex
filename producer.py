...
def intraday_data(freq, market_hours, current_datetime, source, tokens, economic_data, cot=False, vix=False, request=None,\
    ...
    while (current_datetime >= market_hours['market_start']) and (current_datetime <= market_hours['market_end']):
        ...
        try:
            ...
            # Update current time
            current_datetime = pytz.utc.localize(datetime.datetime.now(datetime.timezone.utc)).astimezone(time_zone['EST'])
        ...
        logging.warning(f'Current time: {datetime.datetime.strftime(current_datetime, "%Y-%m-%d %I:%M %p")} {current_datetime.tzname()}')
